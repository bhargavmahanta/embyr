...
#60 implements the repeatable-read snapshot and objective projection. #61 implements the initial retrieval policy, validates its limits, and freezes the document refresh/input-version process. #62 integrates unchanged M3 scoring and explanation mechanics, selecting the primary entity domain with a challenge-state row (or the lexicographically first matching domain when no primary has state) for M3's one-area difficulty-fit input. #63 owns the nullable `reason_code` migration and compact selected-result persistence. #64 implements both endpoints and the `distance-band/v1` mapping. #65 validates the targeted flow; #66 documents the final architecture and limits.
